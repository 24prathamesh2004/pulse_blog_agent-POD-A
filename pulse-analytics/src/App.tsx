...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* Header */}
      <header className="sticky top-0 z-40 border-b bg-[var(--background)]/90 backdrop-blur-md" style={{ borderColor: "var(--rule)" }}>
        <div className="mx-auto flex max-w-[1400px] items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            {source && (
              <span className="hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border" style={source === "live" ? { background: "oklch(0.45 0.16 155 / 0.12)", color: "oklch(0.45 0.16 155)", borderColor: "oklch(0.45 0.16 155 / 0.3)" } : { background: "oklch(0.65 0.18 75 / 0.12)", color: "oklch(0.55 0.18 75)", borderColor: "oklch(0.55 0.18 75 / 0.3)" }}>
            ...
            <button onClick={load} disabled={loading} className="grid h-9 w-9 place-items-center rounded-full border hover:bg-[var(--muted)] transition-colors disabled:opacity-40" style={{ borderColor: "var(--rule)" }}>
              <RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />
            </button>
            <button onClick={toggle} className="grid h-9 w-9 place-items-center rounded-full border hover:bg-[var(--muted)] transition-colors" style={{ borderColor: "var(--rule)" }}>
      ...
      <main className="flex-1 mx-auto w-full max-w-[1400px] px-6 py-10">
        <header className="mb-10">
          ...
          <p className="text-sm max-w-2xl" style={{ color: "var(--muted-foreground)" }}>
            Real-time insights into the AI-powered newsroom. All content is researched, written, and curated by autonomous agents.
          </p>
        </header>
...
function Dashboard({ data }: { data: AnalyticsData }) {
  return (
    <div className="space-y-8">
      ...
      {/* Publishing velocity + Category pie */}
          ...
          <div className="space-y-2 max-h-72 overflow-y-auto mt-2">
            {data.sourceStats.length === 0
              ...
              : data.sourceStats.slice(0, 12).map((s, i) => (
                <div key={i} className="flex items-center justify-between p-3 rounded-lg border hover:bg-[var(--muted)] transition-colors" style={{ borderColor: "var(--rule)" }}>
                  <div className="min-w-0">
                    <div className="text-sm font-medium truncate">{s.name}</div>
                    <div className="meta text-xs">{s.category}</div>
                  </div>
                  <div className="text-right ml-4">
                    <div className="text-lg font-semibold" style={{ fontFamily: "Fraunces, serif" }}>{s.articles}</div>
        ...
        <Card title="Ask the Archive" sub="RAG question analytics">
          <div className="flex flex-col items-center justify-center py-8 gap-6">
            ...
            <div className="grid grid-cols-2 gap-6 w-full border-t pt-6" style={{ borderColor: "var(--rule)" }}>
              <div className="text-center">
                <div className="text-3xl font-semibold" style={{ fontFamily: "Fraunces, serif", color: "var(--primary)" }}>{data.ragStats.questionsLast7Days}</div>
                <div className="meta text-xs mt-1">Last 7 days</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-semibold" style={{ fontFamily: "Fraunces, serif", color: "var(--primary)" }}>{data.summary.totalSources}</div>
  ...
  return (
    <div className="rounded-2xl border p-5 hover:shadow-lg transition-shadow" style={{ borderColor: "var(--rule)", background: "var(--card)" }}>
      <div className="flex items-center gap-3 mb-3">
        <div className="grid h-10 w-10 place-items-center rounded-xl" style={{ background: `${color}20` }}>
