...
export const MOCK_DATA: AnalyticsData = {
  summary: { totalPublished: 347, avgQualityScore: 82, activeStorylines: 14, totalStorylines: 38, totalSources: 22, totalQuestions: 193, totalViews: 48210, avgViewsPerArticle: 139, articlesLast24h: 6, lastPublished: new Date(Date.now() - 1000 * 60 * 47).toISOString() },
  postsByCategory: [
  ...
  publishingVelocity: days.map((day, i) => ({ day, count: Math.max(0, Math.round(8 + Math.sin(i * 0.6) * 4 + (Math.random() * 3 - 1.5))) })),
  sourceStats: [
  ...
  discoveryFunnel: { discovered: 2840, scraped: 2190, approved: 891, published: 347, rejected: 1299, duplicate: 544, conversionRate: "12.2" },
  agentPerformance: { totalRuns: 1024, successfulRuns: 978, failedRuns: 46, successRate: "95.5", avgRunDurationSeconds: 38 },
  qualityDistribution: [{ range: "0-20", count: 2 }, { range: "21-40", count: 8 }, { range: "41-60", count: 31 }, { range: "61-80", count: 142 }, { range: "81-100", count: 164 }],
  ragStats: { totalQuestions: 193, questionsLast7Days: 41 },
  viewsTrend: days.map((day, i) => ({ day, views: Math.max(0, Math.round(1400 + Math.sin(i * 0.4) * 600 + (Math.random() * 300 - 150))) })),
  mostViewedPosts: [{ id: "1", views: 3812 }, { id: "2", views: 2940 }, { id: "3", views: 2105 }],
};
